...
const TopAtists = () => {
  ...
  return (
    ...
    <div className='flex flex-wrap gap-y-6 justify-between'>
       {data?.tracks.slice(0,data.tracks.length-(data.tracks.length%3)).map((Elements, i)=>{
        return <div key={Elements?.artist?.adamid+i} className='flex flex-col  md:w-[30%] p-4 bg-white/5 bg-opacity-80 backdrop-blur-sm rounded-lg cursor-pointer'>
