...
const Controls = ({handleAddFavSong,handleShareClick, mobilePlayerOpen, isPlaying, repeat, setRepeat, shuffle, setShuffle, currentSongs, handlePlayPause, handlePrevSong, handleNextSong }) => (
  <div className={`${mobilePlayerOpen?"w-[80vw] mt-[3vh] justify-between":"justify-around md:w-[25vw] 2xl:w-80"} flex items-center`}>
    ...
    <FaHeart className={`cursor-pointer md:flex ${mobilePlayerOpen?"flex ":"hidden"}`} size={mobilePlayerOpen?30:20} onClick={handleAddFavSong} color={repeat ? 'red' : 'white'} />
